...
import CreateModal from "@/components/cms-admins/create";
import { getCmsAdminsService } from "@/services/cms-admins";
import CmsAdminsTable from "@/components/cms-admins/table";
...
export default function CmsAdminsPage() {
  ...
  const handleSearch = (value: any) => {
    ...
    const filtered = data.filter((cmsAdmin: any) => {
      return (
        cmsAdmin.display_name.toLowerCase().includes(lowercasedValue) ||
        cmsAdmin.cms_role.toLowerCase().includes(lowercasedValue) ||
        cmsAdmin.email.toLowerCase().includes(lowercasedValue)
      );
    });
                  ...
                  {(user?.cms_role === undefined ||
                    ...
                    <Button
                      ...
                      onClick={() => {
                        exportFromJSON({
                          data: filteredData,
                          fileName: `cms-admins-data(${moment().format(
                            "YYYY-MM-DD"
                          )})`,
                          exportType: exportFromJSON.types.csv,
                          fields: ["display_name", "cms_role", "email"],
                        });
                      }}
                    >
                      Export
