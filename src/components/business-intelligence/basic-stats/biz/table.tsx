...
export default function BizTable({ data }: BizTableProps) {
  const {
    token: { colorPrimary },
  } = theme.useToken();
  ...
  const bizEntries = data?.biz;
  const flattenBizData = flattenData(bizEntries);

  const clinics =
    flattenBizData.clinics?.map((clinic: any, index: number) => ({
      ...clinic,
  ...
  return (
    <>
      <Row gutter={[16, 16]}>
        <Col flex="100%">
          <Card title="Biz Plan" styles={{ title: { color: colorPrimary } }}>
            <Row gutter={[16, 16]}>
              <Col xs={12} sm={12} md={12} lg={8} xl={8} xxl={8}>
                <Statistic
                  title="Total Clinics"
                  value={flattenBizData.total_clinics}
                />
              </Col>
              <Col xs={12} sm={12} md={12} lg={8} xl={8} xxl={8}>
                <Statistic
                  title="Total Clinic Admins"
                  value={flattenBizData.total_clinic_admins}
                />
              </Col>
              <Col xs={12} sm={12} md={12} lg={8} xl={8} xxl={8}>
                <Statistic
                  title="Total Prac9"
                  value={flattenBizData.total_prac9}
                />
              </Col>
              <Col xs={12} sm={12} md={12} lg={8} xl={8} xxl={8}>
                <Statistic
                  title="Total Patients"
                  value={flattenBizData.total_patients}
                />
              </Col>
              <Col xs={12} sm={12} md={12} lg={8} xl={8} xxl={8}>
                <Statistic
                  title="Total Users"
                  value={flattenBizData.total_users}
                />
              </Col>
            </Row>
          </Card>
