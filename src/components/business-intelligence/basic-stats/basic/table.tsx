...
export default function BasicTable({ data }: BasicTableProps) {
  ...
  const basicEntries = data?.basic;
  const flattenBasicData = flattenData(basicEntries);

  const clinics =
    flattenBasicData.clinics?.map((clinic: any, index: number) => ({
      ...clinic,
  ...
  return (
    <>
      <Row gutter={[16, 16]}>
        <Col flex="100%">
          <Card title="Basic Plan" styles={{ title: { color: colorPrimary } }}>
            <Row gutter={[16, 16]}>
              <Col xs={12} sm={12} md={12} lg={8} xl={8} xxl={8}>
                <Statistic
                  title="Total Clinics"
                  value={flattenBasicData.total_clinics}
                />
              </Col>
              <Col xs={12} sm={12} md={12} lg={8} xl={8} xxl={8}>
                <Statistic
                  title="Total Clinic Admins"
                  value={flattenBasicData.total_clinic_admins}
                />
              </Col>
              <Col xs={12} sm={12} md={12} lg={8} xl={8} xxl={8}>
                <Statistic
                  title="Total Prac9"
                  value={flattenBasicData.total_prac9}
                />
              </Col>
              <Col xs={12} sm={12} md={12} lg={8} xl={8} xxl={8}>
                <Statistic
                  title="Total Patients"
                  value={flattenBasicData.total_patients}
                />
              </Col>
              <Col xs={12} sm={12} md={12} lg={8} xl={8} xxl={8}>
                <Statistic
                  title="Total Users"
                  value={flattenBasicData.total_users}
                />
              </Col>
            </Row>
          </Card>
