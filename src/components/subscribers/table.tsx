...
import { deleteSubscriberService } from "@/services/subscribers";
...
export default function SubscribersTable({
  ...
  const handleMenuClick = (key: string, row: any) => {
    switch (key) {
      case "1": // View
        router.push(`/user/subscribers/${row.id}/`);
        break;
      case "2": // Edit
  ...
  const deleteMutation = useMutation({
    mutationFn: (value: any) => deleteSubscriberService(value),
  });
        ...
        deleteMutation.mutate(subscriberId, {
          ...
          onSuccess: () => {
            ...
            setFilteredData((prev: any) =>
              prev.filter((item: any) => item.id !== subscriberId)
            );
  ...
  const columns: TableProps<any>["columns"] = [
    {
      ...
      key: "display_name",
      render: (text, record) => (
        <Link href={`/user/subscribers/${record.id}`}>
    ...
    },
    {
      ...
      key: "clinic_name",
      render: (text, record) => (
        <Link href={`/clinics/${record?.clinic_id}`}>
          <Title level={5} style={{ marginBottom: 0, color: colorPrimary }}>
            {text}
          </Title>
        </Link>
