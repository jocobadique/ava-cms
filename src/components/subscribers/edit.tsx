...
import { updateSubscriberService } from "@/services/subscribers";
...
export default function EditModal({ data, isOpen, onClose }: EditProps) {
  ...
  const updateMutation = useMutation({
    mutationFn: (values: any) =>
      updateSubscriberService(values.id, { ...values }),
    onMutate: async () => {
      setLoading(true);
    },
    ...
    onSuccess: async () => {
      message.success({ content: "Subscriber updated successfully." });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Form.Item hidden name="clinic_id">
          <Input type="hidden" />
        </Form.Item>
        <Form.Item hidden name="branch_id">
          <Input type="hidden" />
        </Form.Item>
        <Form.Item hidden name="clinic_role">
          <Input type="hidden" />
        </Form.Item>
        <Row gutter={[16, 0]}>
